        ...
        if (event == "telemetry") {
          ...
          */
          ...
          for(unsigned int i = 0; i < number_of_waypoints; i++) {
            ...
            // Rotate the delta_x and delta_y to the vehicle's coordinate system
            trans_wps_x[i] = delta_x * cos_psi - delta_y * sin_psi; // + 0.0 for x
            trans_wps_y[i] = delta_x * sin_psi + delta_y * cos_psi; // + 0.0 for y
          }

          // Calculate the 3rd order polynomial coefficients
          auto coeffs = polyfit(trans_wps_x, trans_wps_y, 3);

          // Create the state vector
          // x, y and psi are 0.0, since we use the vehicle's coordinate system
          // v is provided by the simulator
          // The cte = polyeval(coeffs, x) - y at x=0.0 and y=0.0 equals polyeval(coeffs, 0.0) - 0.0 ==> coeffs[0]
          // epsi = y  - atan(coeffs[1]) with psi=0.0 equals 0.0 - atan(coeffs[1])
          Eigen::VectorXd state(6);
          state << 0.0, 0.0, 0.0, v, coeffs[0], -atan(coeffs[1]);
          ...
          // Multiply be -1.0 because a positive steering value implies a right turn in the simulator
          msgJson["steering_angle"] = -1.0 * actuators[0] / deg2rad(25);
