 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (unsigned int t = 0; t < N - 1; t++) {
      ...
      fg[0] += 45 * CppAD::pow(vars[a_start + t], 2);
    }
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // Lower limit for the initial states
  ...
  // Upper limit for the initial states
  ...
  // Store the estimated MPC x and y coordinates
  mpc_x_vals.clear();
  mpc_y_vals.clear();
  ...
  return {solution.x[delta_start] / Lf, solution.x[a_start]};
}
